...
void displayResults(Process processes[], int n) {
    printf("Process\tArrival Time\tBurst Time\tEnd Time\tWaiting Time\tTurnaround Time\n");

    for (int i = 0; i < n; i++) {
        printf("P%d\t%d\t\t%d\t\t%d\t\t%d\t\t%d\n", processes[i].id, processes[i].arrivalTime,
               processes[i].burstTime, processes[i].endTime, processes[i].waitingTime, processes[i].turnaroundTime);
    }
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (i < n - 1 && currentTime < processes[i + 1].arrivalTime) {
            int idleTime = processes[i + 1].arrivalTime - currentTime;
    ...
    currentTime = 0;
    for (int i = 0; i < n; i++) {
        printf("\t%d", currentTime + processes[i].burstTime);
        currentTime += processes[i].burstTime;
